...
- A `v0.8.3` tag exists on `origin/instagram-yt-dlp-migration`, while `main` remains at `0.8.2`. Do not attempt to release `0.8.3` from `main`; use `0.8.4+` or reconcile/delete the tag intentionally.
...
- `python -m build` produced both sdist and wheel for 0.8.2.
- `npm run lint` passed.
