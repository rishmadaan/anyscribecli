...
| 0.16.2 | **Voice sweep + polish** — all 104 user-visible runtime strings speak `anyscribe` (matching the docs), `sarvam` accepted as a spelling for the `sargam` provider everywhere a provider name enters (nine call sites, false-green key checks closed), `anyscribe ui` auto-retries up to 10 busy ports (65535-clamped, parser-validated `--port`), grammar fixes, README/landing screenshot downscaled 46% | **Current** |
