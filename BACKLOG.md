...
| 0.14.1 | Audit fixes (independent post-release audit): gpt-transcribe `languages` object parse (wrote a dict into frontmatter), `provider_models:` null-YAML crash, `-m` on local now rejected loudly, OpenAI diarize per spec (`diarized_json` + `chunking_strategy`), canonical turbo repo id, Sarvam `with_diarization` dropped (Batch-only upstream), doc contradiction fixes | **Current** |
