...
|---|---|
| **Find out what scribe will actually run** | `scribe config --json` — settings + `resolved` (provider/model/why) + `providers` (models, keys, tiers). **Start here** for any "what am I using / what can I change" question |
...
| See current config | `scribe config show` (raw settings) or `scribe config --json` (settings + what runs next) |
| Switch provider | `scribe config set provider <name>` (also writes `quality=custom` so it sticks) |
| Add an OpenRouter model to the pickers | `scribe config set extra_models.openrouter "<slug>,<slug>"` (openrouter only) |
...
```bash
scribe config set provider elevenlabs    # Switch provider (also writes quality=custom)
scribe config set quality cost           # Or let a tier pick the provider
...
scribe config set provider_models.groq whisper-large-v3          # Pin a model
scribe config set extra_models.openrouter "qwen/qwen3-omni-flash" # Add an OpenRouter slug
```

**`scribe config set` is one entrypoint for everything** — plain settings, dotted keys (`instagram.browser`, `provider_models.<p>`, `extra_models.openrouter`), and API keys (`<provider>_api_key`, which go to `.env`, never to `config.yaml`). On a bad key or value it exits 1 and prints the valid choices; parse those instead of guessing. `scribe config list-keys --json` enumerates every settable key with its current value.

**Via MCP:** the `set_config` tool covers the same key space (settings, pins, `extra_models.openrouter`, API keys) and returns `{"success": false, "error", "choices"}` on rejection. `get_config` returns all settings plus the resolved workspace path; for what will actually run, use `scribe config --json`.
