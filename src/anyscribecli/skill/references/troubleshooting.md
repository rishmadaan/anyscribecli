...
> A stale `OPENROUTER_MODEL=` line in `~/.anyscribecli/.env` is **not** the cause — that env var was removed in 0.15.0 and is no longer read. Tell the user to delete the line and set `provider_models.openrouter` instead.
