...
from anyscribecli.core import tray as core

console = Console()
err_console = Console(stderr=True)

_MISSING_EXTRA = (
    "The tray companion needs extra packages.\nRun: pip install -U 'anyscribecli[tray]'"
)
...
def tray(
    ...
    Requires the optional extra: [bold]pip install 'anyscribecli\[tray]'[/bold]
    """
    try:
...
def _run_tray(port: int) -> None:
    ...
    def _start_server() -> None:
        ...
        state["proc"] = subprocess.Popen(
            [sys.executable, "-m", "anyscribecli", "ui", "--port", str(port), "--no-open"],
