...
GITHUB_REPO = "https://github.com/rishmadaan/anyscribe.git"
PYPI_PACKAGE = "anyscribe"


def get_install_type() -> str:
    """Detect how anyscribe was installed.
...
def _find_git_repo() -> Path | None:
    ...
    try:
        ...
        pkg_path = Path(anyscribe.__file__).resolve()
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ver_result = subprocess.run(
                ["git", "show", "origin/main:src/anyscribe/__init__.py"],
                cwd=repo_path,
                capture_output=True,
                text=True,
                timeout=10,
...
def _git_update(repo_path: Path, force: bool = False) -> bool:
    ...
    if status_result.stdout.strip() and not force:
        ...
        console.print(
            "Run [bold]anyscribe update --force[/bold] to update anyway (will stash changes)."
        )
        return False
    ...
    version_file = repo_path / "src" / "anyscribe" / "__init__.py"
    new_version = current
...
def _pip_update() -> bool:
    ...
    # Check new version
    ver_result = subprocess.run(
        [sys.executable, "-c", "from anyscribe import __version__; print(__version__)"],
        capture_output=True,
        text=True,
        timeout=10,
    )
...
def check_for_updates(quiet: bool = False) -> bool:
    ...
    if latest and latest != current:
        console.print(
            ...
            f"  Run [bold]anyscribe update[/bold] to update."
        )
        return True
    elif not quiet:
