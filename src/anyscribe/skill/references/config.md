...
> **Web UI parity:** every config key (including `prompt_download`, `local_file_media`, `keep_media`, `instagram.browser`) is editable at `scribe ui` → Settings; the page leads with a "Next run" banner mirroring `scribe config`. When a user asks "where do I change X in the UI", the answer is always Settings. Every *setting* can be changed from either the CLI or the Web UI. A few maintenance commands are CLI-only: `batch`, `logs`, `doctor`, `update`, and `tray`.
...
> **Pre-0.8.3 users:** <!-- version-pin-ok --> the older `instagram.username` field and the
