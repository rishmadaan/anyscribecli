...
from anyscribe.config.paths import TMP_DIR, VIDEO_DIR, AUDIO_DIR
...
def download(
    ...
    Saves to ~/.anyscribe/downloads/video/ or ~/.anyscribe/downloads/audio/.

    [dim]Tip: Wrap URLs in quotes to avoid shell issues.[/dim]
    """
    from anyscribe.cli.transcribe import _read_clipboard, _validate_input
...
def _download_video(url: str, platform: str, tmp_dir: Path, quiet: bool) -> dict:
    ...
    from anyscribe.core.deps import ensure_ytdlp_current, get_command
    from anyscribe.vault.writer import slugify
