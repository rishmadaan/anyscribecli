...
from anyscribe.config.settings import load_config, save_config, load_env, save_env
from anyscribe.providers import PROVIDER_KEY_ENV, list_providers, get_provider
...
def config_set(
    ...
    if key_lower in _API_KEY_MAP:
        ...
        console.print(f"[green]Saved[/green] {env_var} to ~/.anyscribe/.env")
        return
...
def providers_test(
    ...
    if env_var:
        if os.environ.get(env_var):
            ...
            console.print("  Add it to ~/.anyscribe/.env")
            raise typer.Exit(code=1)
